  ...
  return (
    <div className="pb-0">
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center space-y-12">
        ...
        <Reveal stagger className="grid grid-cols-1 sm:grid-cols-3 gap-1">
          <RevealItem>
            <m.button
              onClick={() => setActiveTab('listings')}
              className="group relative aspect-[380/429] overflow-hidden w-full"
              {...liftHover}
            >
              <img
        ...
        {/* 3. AGENT INTRO STRIP - tan/beige, photo-left, matches Canopy's
            exact layout. */}
        <Reveal className="grid grid-cols-1 sm:grid-cols-2 border border-[#C9A96A]/30 text-left sm:h-[560px]">
      ...
      {/* 5.5 MARKET PULSE - The Friedman Market Momentum Index (FMMI), a
          real proprietary weekly score with real sub-component breakdown,
          pulled from src/data/fmmi.ts. Add a new entry there each week and
          the gauge, sub-scores, and trend sparkline all update on their
          own - no other code changes needed. */}
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <Reveal>
          <MarketPulseCard
            reportHref={getLatestFmmi().sourceSlug ? `/blog/${getLatestFmmi().sourceSlug}` : '/blog'}
          />
        </Reveal>
      </section>
